...
public class VorbisEncoder {
    ...
    public static byte[] GenerateFile(float[][] floatSamples, int sampleRate, int channels) {
        ...
        for (int readIndex = 0; readIndex <= floatSamples[0].Length; readIndex += WriteBufferSize) {
            if (readIndex == floatSamples[0].Length) {
                ...
                processingState.WriteData(floatSamples, Math.Min(WriteBufferSize, floatSamples[0].Length - readIndex - 1), readIndex);
            }
